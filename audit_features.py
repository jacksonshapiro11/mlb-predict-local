...
for col in sorted(df.columns):
    ...
    if any(x in col_lower for x in [
        'release_speed', 'release_spin_rate', 'release_pos', 'pfx_x', 'pfx_z', 
        'plate_x', 'plate_z', 'vx0', 'vy0', 'vz0', 'ax', 'ay', 'az', 
        'sz_top', 'sz_bot', 'effective_speed', 'release_extension', 'spin_axis',
        'zone', 'hc_x', 'hc_y', 'launch_speed', 'launch_angle', 'hit_distance',
        'babip_value', 'iso_value', 'woba_value', 'woba_denom',
        'delta_run_exp', 'delta_home_win_exp', 'events', 'description',
        'pitch_name', 'pitch_type'  # These are the actual outcome
    ]):
...
for col in pre_pitch:
    if any(x in col.lower() for x in ['balls', 'strikes', 'outs', 'inning', 'score', 'on_']):
        print(f"      {col}")
...
for col in pre_pitch:
    if any(x in col.lower() for x in ['whiff_30_', 'contact_30_', 'hit_30_', 'xwoba_30_']):
        print(f"      {col}")
...
for col in pre_pitch:
    if any(x in col.lower() for x in ['stand', 'p_throws', 'batter', 'pitcher', 'home_team']):
        print(f"      {col}")
...
for col in pre_pitch:
    if any(x in col.lower() for x in ['balls', 'strikes', 'outs', 'inning', 'score', 'on_',
                                     'usage_', 'v_td_', 'spin_td_', 'whiff_30_', 'contact_30_', 
                                     'hit_30_', 'xwoba_30_', 'whiff_7d', 'velo_7d', 'hit_7d',
                                     'stand', 'p_throws', 'batter', 'pitcher', 'home_team']):
        categorized.add(col)
...
print(f"   They represent historical trends/rates, not current pitch outcomes.") 