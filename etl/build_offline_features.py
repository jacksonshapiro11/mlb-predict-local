...
import sys, os, time, logging, duckdb as db, pandas as pd, numpy as np
...
# -------- constants & helpers -------- #
CANON = ["FF", "SL", "SI", "CH", "CU", "FC", "KC", "SW", "FS"]   # + OTHER


def canon(pt: str) -> str:
    return pt if pt in CANON else "OTHER"
...
def main():
    ...
    if not os.path.exists(xwalk_pq):
        ...
        playerid_lookup("all", "all")[["key_mlbam", "key_fangraphs"]].drop_duplicates() \
            .rename(columns={"key_mlbam": "mlbam", "key_fangraphs": "fg"}).to_parquet(xwalk_pq)
    ...
    keep = [
        ...
        "balls", "strikes",
        "pitch_type", "pitch_name", "release_speed", "effective_speed", "release_extension",
        "release_spin_rate", "pfx_x", "pfx_z", "plate_x", "plate_z", "spin_axis",
        "sz_top", "sz_bot", "zone",
    ...
    raw = (raw
        .merge(xwalk.rename(columns={"mlbam": "batter"}), on="batter", how="left")
    ...
    wide_pit = (
        arsenal
        .pivot_table(index=["mlbam", "game_date"],
                     columns="pt",
    ...
    # ---------- merge everything ---------- #
    data = (raw
        .merge(bat_daily[["mlbam", "game_date", "K_PCT_TD", "K_PCT_30D"]],
               left_on=["batter", "game_date"], right_on=["mlbam", "game_date"], how="left")
        .drop(columns="mlbam")
        .merge(wide_pit, left_on=["pitcher", "game_date"], right_on=["mlbam", "game_date"], how="left")
        .drop(columns="mlbam")
        .merge(wide_hit, left_on=["batter", "game_date"], right_on=["mlbam", "game_date"], how="left")
        .drop(columns="mlbam")
        .merge(wide_count, left_on=["pitcher", "game_date"], right_on=["mlbam", "game_date"], how="left")
        .drop(columns="mlbam")
        .merge(recent_form, left_on=["pitcher", "game_date"], right_on=["mlbam", "game_date"], how="left")
            .drop(columns="mlbam")
        .merge(wide_platoon, left_on=["pitcher", "game_date"], right_on=["mlbam", "game_date"], how="left")
            .drop(columns="mlbam")
        )
