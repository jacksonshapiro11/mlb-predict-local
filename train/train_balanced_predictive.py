...
print("🔧 BALANCED: Keeping historical features, removing only current-pitch measurements")
...
def prep_X_y_balanced(df, target, label_encoders=None):
    ...
    if target == TARGET_XWOBA:
        ...
        target_related = [col for col in df.columns if 'pitch_type_can' in col.lower() and col != target]
    ...
    remaining_cats = {
        ...
        'historical_arsenal': [c for c in X.columns if any(x in c.lower() for x in ['usage_', 'v_td_', 'spin_td_', 'whiff_30_'])],
...
X_val_pt, y_val_pt, _ = prep_X_y_balanced(val, TARGET_PT, label_encoders) if len(val) > 0 else (None, None, None)
...
X_val_x, y_val_x, _ = prep_X_y_balanced(val_ext, TARGET_XWOBA, label_encoders) if len(val_ext) > 0 else (None, None, None)
...
print(f"🎯 Test Results (BALANCED PREDICTIVE - HISTORICAL FEATURES KEPT):")
...
print(f"🎯 Expected realistic accuracy: ~45-65% (balanced approach)")
print(f"🎯 This model uses historical tendencies while avoiding current-pitch leakage") 