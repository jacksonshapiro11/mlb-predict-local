...
print("🔧 ULTRA CLEAN: Removing ALL pitch-type-specific features for pitch type prediction")
...
X_val_pt, y_val_pt, _ = prep_X_y(val, TARGET_PT, label_encoders, for_pitch_type=True) if len(val) > 0 else (None, None, None)
...
def add_pt_probs(df, model, label_encoders, pt_encoder):
    X, _, _ = prep_X_y(df, TARGET_PT, label_encoders, for_pitch_type=True)  # Use same clean features
...
print(f"🎯 Test Results (ULTRA CLEAN - NO DATA LEAKAGE):")
...
print(f"🎯 Expected realistic accuracy: ~30-60% (vs previous 95%)")
print(f"🎯 This should give us proper model performance metrics") 