...
def prep_X_y(df, target, label_encoders=None):
    ...
    if target == TARGET_PT:
        ...
        print(
            f"   Mean: {y.mean():.3f}, Std: {y.std():.3f}, Range: {y.min():.3f}-{y.max():.3f}"
        )
...
if len(X_xwoba) > 100:  # Only test if we have enough samples
    ...
    lgb_train_x = lgb.Dataset(
        ...
        y_train_x,
        categorical_feature=[
            i for i, c in enumerate(X_train_x.columns) if c in CAT_COLS
        ],
        free_raw_data=False,
    )
...
if acc <= 0.95 and ll >= 0.3 and ("rmse" not in locals() or rmse >= 0.01):
