...
if min_support < 0.01:  # Less than 1% effective support
    print(f"⚠️  WARNING: Minimum class support is {min_support:.3f} ({min_support*100:.1f}%)")
    print(f"   Consider increasing lambda to {LAMBDA_DECAY * 0.67:.4f} or stratified weighting")
else:
...
def prep_X_y_final(df, target, label_encoders=None, for_xwoba=False):
    ...
    if target == TARGET_XWOBA:
        ...
        if null_pct > 0.3:  # More than 30% nulls
            print("   🔄 FILTERING to in-play events only (avoiding bias from 0-filling)")
...
print(f"✅ Pitch type model trained! Iterations: {model_pt.num_trees()}, Time: {pt_train_time:.1f}s")
...
X_train_x, y_train_x, _ = prep_X_y_final(train_ext, TARGET_XWOBA, label_encoders, for_xwoba=True)
X_val_x, y_val_x, _ = prep_X_y_final(val_ext, TARGET_XWOBA, label_encoders, for_xwoba=True)
...
print(f"✅ xwOBA model trained! Iterations: {model_x.num_trees()}, Time: {xwoba_train_time:.1f}s")
...
# Test performance  
test_acc, test_ll, test_pt_extra = eval_pitch(model_pt, test, label_encoders, pt_encoder)
...
importance_df_x = pd.DataFrame({
    ...
    'importance': feature_importance_x
}).sort_values('importance', ascending=False)
...
print(f"   - Top-3 accuracy: {test_pt_extra['top3_accuracy']:.1%} (practical relevance)")
print(f"   - xwOBA RMSE: {test_rmse:.4f}")
print(f"   - All safeguards applied ✅")
