...
CAT_COLS        = ["stand", "p_throws", "inning_topbot"]  # REMOVED pitch_type and pitch_type_can
LAMBDA_DECAY    = 0.0012          # ≈ 2-season half-life
PARQUET_GLOB    = "data/features/statcast_*.parquet"
SAVE_DIR        = pathlib.Path("models")
SAVE_DIR.mkdir(parents=True, exist_ok=True)

print("🚀 Starting MLB Prediction Model Training Pipeline (LightGBM - FIXED)")
print(f"📊 Training seasons: {SEASONS_TRAIN}")
print(f"🎯 Targets: {TARGET_PT} → {TARGET_XWOBA}")
print("🔧 FIXED: Removed pitch_type from features to prevent data leakage")
...
def prep_X_y(df, target, label_encoders=None):
    ...
    # CRITICAL: Remove pitch_type to prevent data leakage when predicting pitch_type_can
    drop_cols = [TARGET_PT, TARGET_XWOBA, "w", "game_date", "game_pk", "at_bat_number", 
                 "pitch_number", "inning", "inning_topbot", "batter", "pitcher", 
                 "home_team", "pitch_name", "events", "description", "pitch_type"]  # ADDED pitch_type
...
print("\n🎯 HEAD A: Pitch Type Classification (NO DATA LEAKAGE)")
X_train_pt, y_train_pt, label_encoders = prep_X_y(train, TARGET_PT)
X_val_pt, y_val_pt, _ = prep_X_y(val, TARGET_PT, label_encoders) if len(val) > 0 else (None, None, None)
...
def add_pt_probs(df, model, label_encoders, pt_encoder):
    X, _, _ = prep_X_y(df, TARGET_PT, label_encoders)  # Just to get consistent feature set
...
X_val_x, y_val_x, _ = prep_X_y(val_ext, TARGET_XWOBA, label_encoders) if len(val_ext) > 0 else (None, None, None)
...
print(f"🎯 Test Results (NO DATA LEAKAGE):")
...
model_pt.save_model(SAVE_DIR / "pitch_type_fixed.lgb")
model_x.save_model(SAVE_DIR / "xwoba_fixed.lgb")

# Save encoders
with open(SAVE_DIR / "label_encoders_fixed.pkl", "wb") as f:
    pickle.dump(label_encoders, f)

with open(SAVE_DIR / "pt_encoder_fixed.pkl", "wb") as f:
...
metadata = {
    ...
    "training_info": {
        ...
        "removed_features": ["pitch_type"]
    }
}

with open(SAVE_DIR / "model_metadata_fixed.json", "w") as f:
    json.dump(metadata, f, indent=2)

print("✅ FIXED Models saved successfully!")
print(f"📁 Files saved to: {SAVE_DIR}/")
print("   - pitch_type_fixed.lgb")
print("   - xwoba_fixed.lgb")
print("   - label_encoders_fixed.pkl")
print("   - pt_encoder_fixed.pkl")
print("   - model_metadata_fixed.json")

print("\n🎉 Training pipeline complete (DATA LEAKAGE FIXED)!") 