...
def prep_X_y_ultra_clean(df, target):
    ...
    # Show what's left
    feature_groups = {
        'count': [c for c in X.columns if any(x in c.lower() for x in ['ball', 'strike', 'count'])],
        'general_stats': [c for c in X.columns if any(x in c.lower() for x in ['k_pct', 'hit_', 'contact', 'whiff']) and not any(suffix in c for suffix in pitch_type_suffixes)],
        'situational': [c for c in X.columns if any(x in c.lower() for x in ['vs_l', 'vs_r', 'ahead', 'behind', 'even'])],
        'basic': [c for c in X.columns if c in ['stand', 'p_throws', 'release_spin_rate', 'VELO_7D']]
    }
...
print(f"   Most frequent class: {max(pd.Series(y_train).value_counts()) / len(y_train):.3f}") 