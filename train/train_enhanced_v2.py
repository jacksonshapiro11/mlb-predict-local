...
def create_enhanced_features(df):
    ...
    df["score_diff"] = df["home_score"].fillna(0) - df.get(
        "away_score", pd.Series([0] * len(df))
    ).fillna(0)
    df["close_game"] = (abs(df["score_diff"]) <= 2).astype(int)

    # Advanced count features - handle NAs and use capped values
    df["ahead_in_count"] = (
        (balls_capped < strikes_capped) | ((balls_capped == 0) & (strikes_capped == 2))
    ).astype(int)
    ...
    df["late_inning"] = (
        df.get("inning", pd.Series([1] * len(df))).fillna(1) >= 7
    ).astype(int)
    df["extra_innings"] = (
        df.get("inning", pd.Series([1] * len(df))).fillna(1) >= 10
    ).astype(int)

    print(
        f"✅ Enhanced features created: {len([c for c in df.columns if c in ['count_state', 'high_leverage', 'score_diff', 'close_game', 'ahead_in_count', 'behind_in_count', 'two_strike_count', 'three_ball_count', 'full_count', 'late_inning', 'extra_innings']])} new features"
    )
    return df
...
lgb_train_pt = lgb.Dataset(
    ...
    categorical_feature=[
        i
        for i, c in enumerate(X_train_pt.columns)
        if c in ["stand", "p_throws", "count_state"]
    ],
    free_raw_data=False,
)
...
lgb_val_pt = lgb.Dataset(
    ...
    categorical_feature=[
        i
        for i, c in enumerate(X_val_pt.columns)
        if c in ["stand", "p_throws", "count_state"]
    ],
    free_raw_data=False,
)
...
print(
    f"🔧 Enhanced hyperparameters: lr={params_pt_enhanced['learning_rate']}, leaves={params_pt_enhanced['num_leaves']}, depth={params_pt_enhanced['max_depth']}"
)
...
print(
    f"✅ Enhanced pitch type model trained! Iterations: {model_pt.num_trees()}, Time: {pt_train_time:.1f}s"
)
...
def add_pt_probs(df, model, label_encoders, pt_encoder):
    ...
    proba_df["PT_PROB_FASTBALL_GROUP"] = (
        proba_df["PT_PROB_FF"] + proba_df["PT_PROB_SI"] + proba_df["PT_PROB_FC"]
    )
    proba_df["PT_PROB_BREAKING_GROUP"] = (
        proba_df["PT_PROB_SL"] + proba_df["PT_PROB_CU"] + proba_df["PT_PROB_KC"]
    )
    proba_df["PT_PROB_OFFSPEED_GROUP"] = proba_df["PT_PROB_CH"] + proba_df["PT_PROB_FS"]
    proba_df["PT_PROB_MAX"] = proba_df[
        [f"PT_PROB_{c}" for c in pt_encoder.classes_]
    ].max(axis=1)
    proba_df["PT_PROB_ENTROPY"] = -np.sum(
        proba_df[[f"PT_PROB_{c}" for c in pt_encoder.classes_]]
        * np.log(proba_df[[f"PT_PROB_{c}" for c in pt_encoder.classes_]] + 1e-10),
        axis=1,
    )
...
X_train_x, y_train_x, _ = prep_X_y_enhanced(
    train_ext, TARGET_XWOBA, label_encoders, for_xwoba=True
)
X_val_x, y_val_x, _ = prep_X_y_enhanced(
...
lgb_train_x = lgb.Dataset(
    ...
    categorical_feature=[
        i
        for i, c in enumerate(X_train_x.columns)
        if c in ["stand", "p_throws", "count_state"]
    ],
    free_raw_data=False,
)
...
lgb_val_x = lgb.Dataset(
    ...
    categorical_feature=[
        i
        for i, c in enumerate(X_val_x.columns)
        if c in ["stand", "p_throws", "count_state"]
    ],
    free_raw_data=False,
)
...
print(
    f"✅ Enhanced xwOBA model trained! Iterations: {model_x.num_trees()}, Time: {xwoba_train_time:.1f}s"
)
...
val_acc, val_ll, val_pt_extra = eval_pitch_enhanced(
    model_pt, val, label_encoders, pt_encoder
)
val_rmse, val_x_extra = eval_xwoba_enhanced(model_x, val, label_encoders, pt_encoder)

# Test performance
test_acc, test_ll, test_pt_extra = eval_pitch_enhanced(
...
metadata = {
    ...
    "performance": {
        "validation": {
            ...
            "class_accuracy": {
                k: float(v) for k, v in val_pt_extra["class_accuracy"].items()
            },
        },
        "test": {
            ...
            "class_accuracy": {
                k: float(v) for k, v in test_pt_extra["class_accuracy"].items()
            },
        },
    },
    "model_params": {"pitch_type": params_pt_enhanced, "xwoba": params_x_enhanced},
...
print("   - Enhanced features and hyperparameters applied ✅")

if test_acc >= 0.60:
    print("   🎯 TARGET ACHIEVED! Mid-60s accuracy reached! 🚀")
else:
    print("   📈 Progress made, consider additional enhancements for 60%+ target")
