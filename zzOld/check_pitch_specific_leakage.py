...
def analyze_pitch_specific_features(df):
    ...
    for feat in remaining_features:
        ...
        # Current pitch location - MAJOR LEAKAGE
            ...
            x in feat_lower for x in ["zone", "plate_x", "plate_z", "hc_x", "hc_y"]
        ):
        ...
        # Game situation - OK
            ...
            x in feat_lower for x in ["balls", "strikes", "outs", "inning", "score"]
        ):
    ...
    for category, features in leakage_categories.items():
        if features:
            is_leakage = category in [
                "CURRENT_PITCH_PHYSICS",
                "CURRENT_PITCH_LOCATION",
                "CURRENT_PITCH_OUTCOME",
            ]
...
print(
    f"Example pitch: {sample_pitch['pitch_type']} thrown by pitcher {sample_pitch['pitcher']}"
)
