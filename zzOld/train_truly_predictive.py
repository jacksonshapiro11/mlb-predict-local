...
X_train_pt, y_train_pt, label_encoders = prep_X_y_predictive(
    train, TARGET_PT, for_pitch_type=True
)
X_val_pt, y_val_pt, _ = (
    prep_X_y_predictive(val, TARGET_PT, label_encoders, for_pitch_type=True)
    if len(val) > 0
    else (None, None, None)
)
...
X_val_x, y_val_x, _ = (
    prep_X_y_predictive(val_ext, TARGET_XWOBA, label_encoders)
    if len(val_ext) > 0
    else (None, None, None)
)
...
print("🎯 Test Results (TRULY PREDICTIVE - NO CURRENT-PITCH DATA):")
...
print("🎯 Expected realistic accuracy: ~25-45% (much lower but truly predictive)")
print("🎯 This model can make real-world predictions BEFORE the pitch is thrown")
