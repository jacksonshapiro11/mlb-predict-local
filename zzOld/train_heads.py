...
def eval_pitch(model, df):
    ...
    acc = (
        pred.argmax(1) == y.map({cls: i for i, cls in enumerate(unique_classes)})
    ).mean()
...
try:
    ...
    onnx_pt = convert_lightgbm(
        model_pt,
        initial_types=[
            ("float_input", FloatTensorType([None, len(model_pt.feature_name())]))
        ],
    )
    ...
    onnx_x = convert_lightgbm(
        model_x,
        initial_types=[
            ("float_input", FloatTensorType([None, len(model_x.feature_name())]))
        ],
    )
